...
- **`providers.py`** — extra sources, each active only when its env keys exist, each fail-soft: **USAJobs** (`USAJOBS_API_KEY` + `USAJOBS_EMAIL`; salaries always employer-stated; PH/PA rate codes), **Jooble** (`JOOBLE_API_KEY`; salary is unflagged free text → always "Pay not listed"), **JSearch** (`JSEARCH_API_KEY`; 200 req/MONTH hard cap → fixed 5-query budget per scan, never per-title; prefers `apply_options[].is_direct` employer/ATS links). Specs were verified against official docs 2026-06-10 — re-verify before changing params.
